var servidor = 'http://localhost/numerica';
	...
	/*
	var uploader = null;
	mozillaUniverse.input = null;//ultra insegurísmo
	*/
		...
		if(!mozillaUniverse.exists)
		{
			...
			*/
				...
				*/
				...
				{
					mozillaUniverse.getScript(this.servidor+'/js/excanvas.js');
				}
			
			//the absolute must haves!
			mozillaUniverse.getScript(this.servidor+'/js/jit.min.js');
	...
	mozillaUniverse.widget = function(options){
		...
		*/
		...
		jQuery('head').append('<link rel="stylesheet" href="'+servidor+'/css/universe.css" type="text/css">');
		...
		var $mozillaUniverse_footnote = jQuery('<span id="mozilla-universe-footnote">'+ options.widget.footnote +'</span>');
		...
		*/
		switch(options.widget.theme)
		{
			...
			case 'tab':
				...
				*/
				...
				$mozillaUniverse_container.append('<div id="mozilla-universe-toggle"><div id="mozilla-universe-tab"><img src="'+servidor+'/img/numerica-tab.png" alt="mozilla"></div></div>'); //TODO numerica tab
			...
			case 'footer':
				...
				*/
				...
				jQuery('.mozilla-universe-overlay').append('<img id="mozilla-universe-close" src="/universe/assets/img/mozillaUniverseWidget-close.png" alt="close">');
		...
		/*
		Eventos comunes independientemente del tipo de widget
		*/
		jQuery('[rel=marcar-nuevo]').click(function(){
			var hash   = location.hash;
			var rootz = (hash.substring(0, 1) == '#') ?
				hash.substring(1) : hash;					
			var url = prompt("URL"); //var url = document.location.href; TODO
			if(url === null) return;
			var id = "editar"; /**GUID*/
			var descript = "editar";
			var nombre = "editar";
...
*/
		...
		org.mozilla.SiteMap = {
			...
			defaultConfig:
			{
				...
				onPlaceLabel: function(domElement, node)
				{
					...
					if(typeof node.data.icon !== 'undefined' && node.data.icon !== "" && domElement.getElementsByTagName('img').length === 0) {
						$domElement
							.addClass('imaged')
							.prepend(
								'<span class="logo"><img src="' + node.data.icon +'"/></span>');
						node.data.$dim = 0;
					}
			...
			showTip: function(node, block)
			{
				...
				if (typeof img !== 'undefined' && img != "" && img != null) {
					tipContent += '<img src="' + node.data.image + '" ' +
						'alt="' + node.name + '" class="panel-foto"/>';
				}
				else{
					sinrostro = true;
				}
				...
				if (typeof node.data.url !== 'undefined') {
					tipContent += '<a href="' + node.data.url + '" ' +
						...
						node.data.url.replace('http://', '') + '</a><br/>';
				}
				if (typeof node.data.desc !== 'undefined') {
					tipContent += '<p class="panel-desc">' + node.data.desc.join('</p><p>') + '</p><br/>';
				}
				...
				//foto
				jQuery(".panel-foto").click(function(){
					var input = document.createElement('input');
					input.type = 'file';
					input.click();
					mozillaUniverse.input = input;
					uploader.setParams({
						coordenada : node.id
					});
					mozillaUniverse.uploadRecursivo(0);//TODO: asincronidad, ver abajo (feo)
				});
				...
				jQuery(".panel-desc").dblclick(function(){
					...
					if(acrobat.length == 0){
						...
						jQuery(".panel-desc").html("<textarea class=\"panel-desc-area\">"+node.data.desc+"</textarea>");
						//enter -> val() -> update!
						jQuery(".panel-desc-area").keydown(function(event){
							if(event.keyCode == 13){
								...
								// ....
								jQuery(".panel-desc").empty();
								jQuery(".panel-desc").text(acid); //query
							}
						});
					}
				});
				...
				jQuery(".titulo").dblclick(function(){
					...
					jQuery(".panel-name").keydown(function(event){
						if(event.keyCode == 13){
							...
							jQuery(".titulo").empty();
							jQuery(".titulo").html(nombre);
							org.mozilla.SiteMap._init();
						}
					});
				});
			...
			/**
			 * SI EL REQUEST TIENE EXITO, SE LIMPIA CALCULANDO LA PROFUNDIDAD
			...
			loadGraph: function()
			{
				...
				// load data
				org.mozilla.SiteMap.hypertree.loadJSON(org.mozilla.SiteMap.data); //LA DE cleanData
	...
	*/
	...
	//TODO: inseguro
	function ajaxUpload(){
		var elemental = document.getElementById("file-uploader"); //ahorrable?
	...
	//TODO: insegurísimo
	/**
	 * Lo estoy separando por asincronidad, si no envía el archivo antes que el input lo tenga
	 */
	mozillaUniverse.upload = function(){
		var input = mozillaUniverse.input;
	 ...
	 * con recursividad
	 */
	mozillaUniverse.uploadRecursivo = function(tiempo){
		var input = mozillaUniverse.input;
		if(input.files.length==0){
			if(tiempo <= 5000){
				setTimeout('mozillaUniverse.uploadRecursivo('+ (tiempo + 200) +')', 200);
			}
			else{
				console.log('No se pudo subir la foto');
				return;
			}
		}
		else{
